    ...
    if (Array.isArray(certs) && certs.length > 0) {
      ...
      if (certGrid) {
        certGrid.innerHTML = certs.map((c, i) => {
          const imgUrl = c.image && c.image.startsWith('media/') ? c.image : (c.image || '');
          const pdfUrl = c.pdfLink && c.pdfLink.startsWith('media/') ? c.pdfLink : (c.pdfLink || '');
          return `
          <div class="glass-card cert-card reveal reveal-delay-${i % 3}">
            ...
            <div class="cert-info">
              ...
              <p class="cert-issuer"><i class="bi bi-building me-1"></i>${esc(c.issuer)} ${c.year ? '• ' + esc(c.year) : ''}</p>
      ...
      if (awardGrid) {
        awardGrid.innerHTML = awards.map((a, i) => `
          <div class="glass-card award-card reveal reveal-delay-${i % 3}">
            ...
            <p class="award-org">${esc(a.org)} ${a.year ? '• ' + esc(a.year) : ''}</p>
